{{--
    Vendored copy of Laravel Horizon's layout (laravel/horizon v5.47.x). The only change from the package original is how
    the dashboard JavaScript is loaded: Horizon::js() inlines the full ~1.4 MB bundle into this HTML, which pushes the
...
<head>
    ...
    <script>
        window.Horizon = {{ \Illuminate\Support\Js::from(\Laravel\Horizon\Horizon::scriptVariables()) }};
    </script>
    <script type="module" src="{{ asset('vendor/horizon/app.js') }}"></script>
</head>
<body>
