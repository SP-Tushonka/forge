...
final class CommentReplyNotification extends Notification implements Presentable, ShouldQueue
{
    use Queueable;
    ...
    public function toMail(User $notifiable): NotificationMailMessage
    {
        ...
        return (new NotificationMailMessage)
            ->subject($subject)
            ->greeting($subject)
            ->line(sprintf('**%s** replied to your comment on %s **%s**.', $commenterName, $commentableType, $commentableTitle))
            ->line('')
