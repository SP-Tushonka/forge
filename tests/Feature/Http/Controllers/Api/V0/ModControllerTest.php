...
describe('index', function (): void {
    ...
    it('sorts mods by endorsements_count descending', function (): void {
        SptVersion::factory()->state(['version' => '3.8.0'])->create();

        $modLow = Mod::factory()->hasVersions(1, ['spt_version_constraint' => '3.8.0'])->create(['endorsements_count' => 3]);
        $modHigh = Mod::factory()->hasVersions(1, ['spt_version_constraint' => '3.8.0'])->create(['endorsements_count' => 200]);
        $modMid = Mod::factory()->hasVersions(1, ['spt_version_constraint' => '3.8.0'])->create(['endorsements_count' => 42]);

        $response = $this->getJson('/api/v0/mods?sort=-endorsements_count');
    ...
    it('returns endorsements_count when requested', function (): void {
        SptVersion::factory()->state(['version' => '3.8.0'])->create();

        Mod::factory()->hasVersions(1, ['spt_version_constraint' => '3.8.0'])->create(['endorsements_count' => 7]);

        $response = $this->getJson('/api/v0/mods?fields=endorsements_count');

        $response
            ->assertOk()
            ->assertJsonPath('data.0.endorsements_count', 7);
    });
...
describe('show', function (): void {
    ...
    it('returns endorsements_count when requested', function (): void {
        ...
        $mod = Mod::factory()->hasVersions(1, ['spt_version_constraint' => '3.8.0'])->create(['endorsements_count' => 7]);

        $response = $this->getJson(sprintf('/api/v0/mod/%d?fields=endorsements_count', $mod->id));
